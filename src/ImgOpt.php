...
class ImgOpt extends Widget
{
     ...
     */
    private function get_or_convert_to_dest_format(string $img, $global_flag, $file_extension, $conversion_function, bool $recreate = false): ?string
    {
        if (($global_flag === true) || $this->disable || !function_exists($conversion_function)) {
            return null;
        }
        ...
        if ($recreate === false && file_exists($output_full_path) && filesize($output_full_path) > 0) {
        ...
        do {
            ...
            try {
                call_user_func($conversion_function, $img, $output_full_path, $quality);
